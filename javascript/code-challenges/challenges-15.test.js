...
const screenForNames = (arr) => arr.filter(strings => /^(Mr.|Mrs.|Ms.|Dr.)\s[A-Za-z]+/.test(strings));
// Solution code here...
...
const toTitleCase = (arr) => {
  let up = arr.map(strings => strings.charAt(0).toUpperCase() + strings.substring(1));
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let luke = arr.find(mass => mass.name === 'Luke Skywalker');
  return arr.filter(mass => parseInt(mass.mass) > parseInt(luke.mass)).map(mass => mass.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  return arr.sort((a, b) => {
    let firstVal = a[property];
    let secondVal = b[property];
