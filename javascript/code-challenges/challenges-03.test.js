...
const typeNum = (arr) => {
  let onlyNums = arr.filter(name => typeof(name) === 'number');
  return onlyNums;
  // Solution code here...
};
...
const containsAnd = (arr) => {
  let and = arr.filter(onlyAnd => onlyAnd.includes('and'));
  return and;
  // Solution code here...
};
...
const oddValues = (arr) => {
  let odds = arr.filter(onlyOdds => onlyOdds % 2 !== 0);
  return odds;
  // % is the divisible sign. help from betterprogramming.pub
  // Solution code here...
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let combinedArr = arr.filter(filter => !forbiddenValues.includes(filter));
  return combinedArr;
  // Solution code here...
};
