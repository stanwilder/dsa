...
function transformToLis(obj) {
  let newArr = [];
  Object.entries(obj).forEach(listItem => newArr.push(`<li>${listItem[0]}: ${listItem[1]}</li>`));
  return newArr;
  // Solution code here...
}
...
const count = (target, input) => {
  // Solution code here...
  let countTwo = 0;
  input.map(nums => nums.forEach(num => target === num ? countTwo++ : countTwo));
  return countTwo;
};
...
const totalSum = (input) => {
  // Solution code here...
  let tot = 0;
  input.forEach(sum => tot += sum.reduce((a, b) => a + b));
  return tot;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(row => {
    return row.filter(cell => {
      return (typeof cell === 'number' && cell % 5 === 0).map(cell => Math.pow(2, cell));
    });
  });
  // Solution code here...
};
...
let findShortest = (data) => {
  let short = data.sort((a,b) => a.height-b.height);
  return short[0].name;
  // Solution code here...
};
